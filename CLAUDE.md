...
9. 收購定價輔助（定價計算機）：**目標毛利一律對「未稅」售價談**——標價含稅（§6），但那 5% 是代政府收的、不是店家毛利。
   ...
   - `建議上架售價（客人實付） = round_ntd(建議未稅售價 × (1 + tax_rate) ÷ (1 − fee_rate × (1 + tax_rate)))`，**全程只四捨五入一次**（先取整未稅再加稅會多一次捨入誤差）
   - `毛利率 = (未稅實得 − 收購成本) ÷ 未稅實得`，其中 `未稅實得 = round_ntd(含稅 ÷ (1 + tax_rate)) − round_ntd(含稅 × fee_rate)`
   ...
   `fee_rate` 為**行動支付手續費率，取 `linepay_fee_pct` 與 `taiwanpay_fee_pct` 的較大者**——定價當下不知道客人會刷哪一種，取低的會少補。讀不到設定時以 0 計（寧可少補，也不要把客人的價格墊高）。**邊界：`fee_rate × (1 + tax_rate) ≥ 1` 會讓分母 ≤ 0，必須被擋下並回錯。**
   ...
   >
   > 2026-09-09 起再納入行動支付手續費。手續費是店家被金流商抽走的錢，不補進標價目標毛利就達不到：收 1000／毛利 45%／稅 5% 的舊式建議價 1909，客人刷行動支付被抽 42 元後未稅實得只剩 1776，實際毛利 43.7%。採**精確補償**（除以 `1 − fee×(1+tax)`）而非把費率乘上去，後者補償不足。**手續費一律計入標價，付現金的客人也適用同一個標價**（裁示）：一件商品一個價，標籤／POS／客顯才會一致。`fee_rate=0` 時退化為 2026-08-23 的式子。
   >
   > 2026-09-19 起，**要填進「上架售價」的建議價一律無條件進位到 10 的倍數**（21 → 30）：
   > `suggested_listed_price() = round_up_to_listed_step(suggested_price(...))`。毛利式本身
   > （`suggested_price`）不變，維持「全程只四捨五入一次」——進位是價格呈現規則，不是第二次捨入，
   > 分成兩支函式才不會廢掉釘住單次捨入的既有回歸測試。**只約束系統自動帶出的價格**
   > （收購頁套用建議與估計轉售價自動同步、採購頁與餐飲菜單的建議售價）；
   > **店員手動輸入不擋**（特價 99、寄售談定價要填得進去）。
   > 既有商品售價不追溯改動。見 `docs/adr/ADR-023-listed-price-rounded-up-to-ten.md`。
...
## 11. 在店內正式機（MacBook）上工作

- 若你是在**店內 MacBook 正式機**上啟動的 agent：動手做任何事之前，先讀 `docs/41-macbook-lan-tablet-printing.md`。
  其中「讓區網平板也能列印」是**店主已核可、待執行的設定任務**，請照文件做完並回報驗證結果；
  文件裡要求先跟店主確認的事（訪客 Wi-Fi 隔離等），一定要先問。
