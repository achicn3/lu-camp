...
// 2) 採購頁建立一般商品：填進貨成本＋毛利率 → 自動算出建議售價（含稅與行動支付手續費），
...
try {
  ...
  const name = `煙霧濾掛 ${RUN}`;
  await page.fill('input[placeholder="輸入品名或商品編號"]', name);
  await page.waitForTimeout(900);
  await page.click('button:has-text("建立一般商品")');
  await page.waitForSelector('input[aria-label="一般商品進貨成本"]');

  const margin = page.locator('input[aria-label="一般商品預估毛利率"]');
  ok("毛利率預設帶設定值 30%", (await margin.inputValue()) === "30", await margin.inputValue());
  ...
  ok("填成本即自動算出建議售價", Number(auto) > 50, `售價 ${auto}`);
  ...
  ok("毛利率調高 → 建議售價跟著變高", Number(higher) > Number(auto), `${auto} → ${higher}`);
