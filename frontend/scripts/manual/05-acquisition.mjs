...
await page.waitForTimeout(600);
await shot(page, "row-listed-buttons", { locator: '.field:has(input[aria-label="上架售價（含稅與手續費）"])' });
await page.click('button:has-text("套用建議（目標毛利")');
await page.waitForTimeout(400);
const listed = await page.inputValue('input[aria-label="上架售價（含稅與手續費）"]');
note(`套用建議售價 → ${listed}`);
...
await page.fill('input[aria-label="上架售價（含稅與手續費）"]', "4000");
...
await comboPick("分類", "露營裝備", `露營裝備${RUN}`);
await pickGrade(page, "C");
await page.fill('input[aria-label="收購價"]', "300");
await page.fill('input[aria-label="上架售價（含稅與手續費）"]', "800");
await page.click('button:has-text("送出收購")');
await page.waitForSelector(".acq-result", { timeout: 20000 });
await page.waitForTimeout(600);
