// 門市活動 v2 P2 煙霧（docs/40）：在畫面上建立「指定特價 690、只限某品牌」的活動並啟用，
// 同時另有一個全館九折；以結帳試算確認：該品牌商品特價 690（比九折划算），其他商品九折。
...
const SHOTS = process.env.SMOKE_SHOTS ?? join(homedir(), "tmp", "lu-camp-shots", "campaigns-kinds");
const RUN = String(Date.now()).slice(-6);
const BRAND = `Snow Peak-${RUN}`;
const OTHER = `Coleman-${RUN}`;
const CAMPAIGN = `${BRAND} 特價 690`;
...
async function apiJson(path, { method = "GET", token, body } = {}) {
  const response = await fetch(`${API}${path}`, {
    ...
    headers: {
      ...
      ...(method === "POST" ? { "Idempotency-Key": `ck-${RUN}-${Math.random()}` } : {}),
    },
...
try {
  ...
  await page.getByLabel("指定特價").check();
  await page.getByLabel("特價（含稅，元）").fill("690");
  ...
  ok("清單顯示特價與指定範圍", rowText.includes("特價 $690") && rowText.includes(`只限：${BRAND}`), rowText.replace(/\s+/g, " "));
  ...
  ok(
    "指定品牌的商品特價 690（比九折划算）",
    spLine.unit_price === "690" && spLine.campaigns.length === 1 && spLine.campaigns[0].name === CAMPAIGN,
