...
#### 收購頁失敗處理（與 T7 後端對齊，本支不實作 UI）

後端 `POST /acquisitions` 為**單交易、整筆成立或整筆回復**：任一步失敗都不會留下「庫存建了但現金沒扣」之類的半套。前端據此設計，務求「**給現金永遠在系統成功之後**」：

1. **送出前先擋（少打一次後端、給店員即時回饋）**：
   - 賣方/寄售人**姓名與身分證字號必填**；缺任一不可送出（鏡像後端，後端為最終權威——缺 national_id 後端回 `422`）。
   - 付現類型（買斷/散裝）**必須已開帳**：未開帳時前端直接提示「請先開帳」並導向開帳，不送出（後端未開帳回 `409`）。
2. **流程順序（關鍵）**：先 `POST /acquisitions`，**待後端回成功後**，才在畫面提示「**應付現金 $X**」並呼叫硬體代理開錢櫃／請店員付款。**絕不可**先給現金或先開櫃再送出——避免系統失敗卻已付錢的不可逆損失。
3. **失敗呈現（可辨識、可安全重做）**：依後端 `{ error: { code, message } }` 以 inline／toast 顯示**看得懂的原因**（缺什麼、為何擋下，如「賣方未填身分證字號」「尚未開帳」「抽成需 0–100」）。因整筆已回復，店員修正後可**安全地重送**，不會產生半套資料。
4. **成功呈現（避免重做兩次）**：成功後顯示**收購單號（acquisition id）**與待印識別碼（item_code／lot_code）。店員以單號確認此筆已成立，**避免因不確定而重複送出**造成重複入庫／重複付現。
5. **實體現金與系統不一致的兜底**：若已開錢櫃／已付現但對列印或後續有疑慮，現金真實與系統的差異**由 T6 現金對帳（開帳/結帳 expected vs counted、variance）在結帳時兜底**，不在收購頁即時強求兩者一致。
