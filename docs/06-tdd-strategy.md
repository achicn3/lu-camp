...
9. **外部發票佇列**：模擬 Amego 拒絕、傳輸中斷與曖昧回應；明確拒絕進 `FAILED` 可重試，
   未知結果維持已認領 `PENDING` 並先查詢對帳，銷售本身不回滾。
10. **稽核完整**：作廢、改價、現金調整、權限/設定變更、PII 查看都產生稽核紀錄。
11. **定價輔助**：目標毛利對**未稅**售價計（ADR-016）——`建議未稅售價 = 收購價 ÷ (1 − margin_pct/100)`、`建議上架售價（含稅）= round_ntd(建議未稅售價 × (1 + tax_rate))`，只四捨五入一次；`margin_pct` 邊界 **0–99**（=100/>100 須被擋下回錯）、`tax_rate` 邊界 0 ≤ rate < 1 且取自 settings；店員可手動覆蓋。
