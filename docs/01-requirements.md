...
### B. Contacts / 聯絡人主檔（統一）
- 單一 `contact` 主檔，身分只有兩種：`MEMBER`（會員，**每個人建檔就是會員**）與 `SELLER`
  （賣方，賣過東西給店裡的人，由收購流程自動標記）。同一人多次往來只建一筆。
  身分證字號只有賣方需要——純消費會員不必留（2026-09-01 裁示；原本的 `CONSIGNOR`
  已併入 `SELLER`：商品是買斷還是寄售是**商品的屬性**，不是人的屬性）。
