...
2. **Codex review** 審查 diff，對照 `docs/08` 檢查清單；一般 task 跑 `/codex:review --base main`，金額/發票/現金/PII/併發/rollback 等高風險 task 跑 `/codex:adversarial-review --base main`。問題須先修完並重跑到無重大意見。

**合併前提＝本機四道門全綠 且 Codex review 無重大意見**。任一不滿足不得合併。
...
- **子代理並行**：在實作型子代理（`tdd-implementer`）的 frontmatter 加 `isolation: worktree`，主 session 即可同時派多個子代理，各自在隔離 worktree 的獨立分支上實作、commit，互不干擾。審查改依 `CLAUDE.md` 使用 Codex review，不再以 `code-reviewer` 子代理作為合併門。
...
並行合併紀律：每條分支合併**前**都要 `git fetch && git rebase origin/main`（因為 main 會隨其他分支合併而前進），通過本機關卡與 Codex review 後，**依相依順序逐一合併**（被依賴者先合併；後者 rebase 到含前者的最新 main 再合）。一次只合一條、合完即 push，避免互相覆蓋。

## 5. 與既有審查層的關係（見 `docs/08`）
- Layer 1 自動門 = **本機四道門 + 合約漂移**必過（客觀硬底線，於本機執行）。
- Layer 2 AI 審查 = 依 `CLAUDE.md` 跑 Codex review；**這是合併核准關**（取代人工核准）。
...
- Phase 收尾：確認該 Phase 所有分支已（本機關卡綠 + Codex review 無重大意見）合併進 `main`、已 push remote、可 `docker compose up`。
