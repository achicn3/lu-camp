...
POST   /api/v1/sales
       ...
       效果: 建 sale + sale_line、序號品 -> SOLD、散裝 -> 扣 bulk_lot.remaining_qty(以該堆 unit_price)、
             stock_movement(OUT)、依 tenders 寫現金／購物金／行動支付、寄售品 -> 建
             consignment_settlement(PENDING)；若 einvoice_enabled -> 產 invoice + 排 upload queue，
             sale commit 後由 POS 呼叫 /einvoice/sales/{sale_id}/issue
...
GET    /api/v1/einvoice/queue?status=&limit=&offset=  # MANAGER：持久佇列狀態
POST   /api/v1/einvoice/sales/{sale_id}/issue     # POS：Amego 開立；冪等且未知結果先對帳
POST   /api/v1/einvoice/queue/{id}/send           # MANAGER：送 PENDING 開立/作廢/折讓
POST   /api/v1/einvoice/queue/{id}/retry          # MANAGER：FAILED -> PENDING，新交付世代
POST   /api/v1/einvoice/queue/{id}/result         # MANAGER/importer：記錄外部結果事件
```
