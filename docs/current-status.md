...
> Snapshot: 2026-07-22 (`main` @ `b38fbef`). This file is the planning index; source of truth
...
| Active product phase branch | None on this `main` snapshot. Old v0.0.1 hardening worktrees/branches were explicitly abandoned on 2026-07-16; do not merge them as future work. |
...
|---|---|---|
| Foundation / local gates | Implemented. `make check` / `check.sh` / `check.ps1` are the local gate entry points. The repository still has a known 67-file Ruff formatter baseline; lint/type/test gates pass. | `CLAUDE.md`, docs/06, docs/08, docs/11, docs/12 |
...
| Sales / POS / tenders | Implemented: mixed carts, discounts, idempotency, cash/store-credit/Taiwan Pay/LINE Pay, signing, void, returns, receipt/detail printing, and responsive POS UI. | docs/10, docs/16, docs/21, docs/23, docs/30 |
| Store credit / points | SC-1–SC-5 implemented: insert-only ledger, payout/redemption, reversals, reports, settings/history, and deterministic suggestion engine. Return point clawback is proportional. | docs/16, ADR-012, storecredit tests |
...
| E-invoice | Amego B2B/B2C issue/void/allowance/query, persistent delivery state, POS fields, and EPSON proof print are implemented. The old self-hosted Turnkey plan is superseded. | docs/24, einvoice module, `einvoice-smoke.mjs` |
...
- **Unmerged optional UX**: `feat/pos-store-credit-amount` contains a typed partial store-credit amount
  UI but was never merged. Current `main` supports full store-credit or mixed payment by entering the
  cash leg. Rebase/review the old branch rather than assuming its behavior is live.
