...
- **Decision**:
  ...
  - `core/money.py` 提供並測試：`round_ntd()`、`split_tax_inclusive(total, rate) -> (net, tax)`、`commission(gross, pct) -> amount`、定價 `suggested_price(cost, margin_pct, tax_rate)`（**2026-08-23 起目標毛利對未稅售價計，見 ADR-016**；原式為 `round_ntd(cost / (1 − margin_pct/100))`）。**禁用 float。**
