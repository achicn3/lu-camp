...
- **Status**: Accepted；其中「定價輔助」的公式已於 2026-08-23 由 **ADR-016** 取代（目標毛利改以未稅售價計）
...
- **Decision**:
  ...
  - **定價輔助**：主算法用目標毛利率。~~`建議售價 = round_ntd(收購價 ÷ (1 − margin_pct/100))`，為含稅整數元~~ **（此式已由 ADR-016 取代：目標毛利對未稅售價計，`建議上架售價（含稅）= round_ntd(收購價 ÷ (1 − margin_pct/100) × (1 + tax_rate))`）**（沿用 `core/money`，見 ADR-007）；`default_margin_pct` 放 `settings`（整數百分數，預設 45），**`margin_pct` 限 0–99**，超出須擋下回錯。同時顯示該型號歷史售價；店員可手動覆蓋任一數字。
