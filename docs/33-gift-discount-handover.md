...
1. ~~舊版 cart-v1 快照的相容轉換~~ → **裁示不做（2026-08-03）**
   購物車是 30 分鐘過期的暫存草稿，升級時機由店家自己決定。單店單機、關店後升級，
   機器上不會有未結完的單，這段程式永遠不會被執行到。
   **以操作習慣取代程式：升級前先確認沒有結到一半的單。**
   （同 `docs/32 §9` 的取捨精神：能靠常識避開的情況不寫程式。）
2. **退貨同意書以兩個時間點的狀態組內容**：逐行金額與總額分別查詢，同一張單若有併發退貨
   可能來自不同快照。單店單機碰不到。
3. **庫存價值匯出的其餘欄位**仍為舊口徑。
4. ~~紙本收據只有單元測試，沒有實機列印驗證~~ → **已於 2026-08-03 接 EPSON TM-T82III 實印驗證**
   （明細聯 #9002）。實機一印就抓到單元測試看不出來的問題：贈品標記接在品名後面會被
   **欄寬截斷**（單元測試只驗「字串含該標記」，截斷發生在排版進固定欄寬時）。
   已改為獨立子列，見 `bc028c4`。**教訓：排版類的正確性，單元測試擋不住，必須實機印。**
