...
| 一般商品成本 | 新增 `catalog_products.unit_cost`，可填 | 未填則贈品成本顯示為 0（沿用「成本未知不假造」口徑） |
...
`round_ntd` 產生的尾差固定落在**最後一筆**可折行。分攤必須可重現——否則退貨時對不上當初
的金額。
...
- 成本一律取 `cost_snapshot`；NULL（舊資料或未填成本的一般商品）才回退即時 join，
  沿用既有的「成本未知不假造毛利」口徑。
- 折扣報表用 `applied_amount`，**不事後重算**。
- 作廢的銷售不計入任何一份。
- **退回的贈品不在贈品報表中扣除**——退回另有 `GIFT_RETURN` 庫存異動可查；
  混在一起會讓「這期間送出去多少」這個問題再也答不出來。
...
- 一般商品未填 `unit_cost` 時，贈品成本顯示為 0（成本未知，不假造）。
