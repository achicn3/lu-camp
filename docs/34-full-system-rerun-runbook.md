...
**作廢已改為自動送出**（2026-08-27，ADR-019）：退貨／作廢排進佇列後，背景排程約一分鐘內自動
送交平台；沒送成功的可在 `/einvoice-queue`（發票待處理，限 MANAGER）人工「立即送出」／「重試」。
`09d` 用 `markIssued` 蓋的假號碼不要送。

> 本節原本寫「實際送出是店長在發票佇列手動觸發」——但那個頁面當時並不存在
> （docs/36 §95 明載「不另建發票佇列頁」），前端也沒有任何畫面呼叫送出端點，於是作廢
> 永遠送不出去、平台上的發票仍然有效。ADR-019 補上自動送出與該頁面。
