...
請依序：
1. 在目前 repo 根目錄 `lu-camp/` 建立/確認 monorepo 骨架（backend/ frontend/ hardware-agent/、
   docker-compose.yml、docs/ 已存在），不可另建 `store-system/` 子資料夾、不可拆成多個 repo。
...
5. 防呆驗證：故意寫一段 import 不存在模組的程式，證明 ruff/mypy/本機檢查會紅燈，
   貼出紅燈輸出後再移除。
6. 若需要並行實作，確認 `.claude/agents/` 中的 `tdd-implementer` 可用；審查依
   `CLAUDE.md` 現行規則改用 Codex review 指令，不再以 `code-reviewer` 子代理作為合併門。
7. 版本控制（docs/12）：把專案推到 GitHub 遠端（僅作備份/同步，不跑 CI）、
   ...
   合併規則：feature 先 rebase 到最新 main → 本機四道門全綠 + Codex review 無重大意見 →
   合併回 main → push origin main。注意：Phase 0 這些建置本身也開在 chore/phase0-scaffold 分支、最後合併進 main。

每一步都要貼出實際指令輸出，不接受「應該會過」。完成後等我確認。
```

> 審查流程以 `CLAUDE.md` 的「Code Review 採 Codex」為準；若舊文件仍提到
> `code-reviewer` 子代理，視為歷史文字，不作為當前合併門。
...
5. 依 `CLAUDE.md` 跑 Codex review：一般 task 用 `/codex:review --base main`，高風險 task
   用 `/codex:adversarial-review --base main`；問題修完並重跑到無重大意見。
...
- 任務 A（feat/...）：……
- 任務 B（feat/...）：……
各自完成後依 `CLAUDE.md` 跑 Codex review；合併前各自 git fetch && rebase origin/main，
...
2. 用 Codex review 對整個 Phase 的變更做一次總體審查，對照
...
- **獨立審查**：寫的人不自評，一律依 `CLAUDE.md` 跑 Codex review（人工抽查金額/發票/現金/PII 為選配）。
- **小步可回溯**：一任務一 commit；不確定就停下來問你。

## 你每次回覆它時的口頭禪
- 「先給我紅燈再實作。」
- 「貼出 pytest / ruff / mypy 的實際輸出。」
- 「跑 Codex review 審查這個 diff，對照 docs/08 清單。」
