...
## R7 發票月報（申報用；2026-09-14 起）

**為什麼**：月底申報要逐筆核對銷項／作廢／折讓／進項，原本系統只有「發票待處理」頁，
沒有任何依期間匯出的清單，會計每個月得自己翻資料庫。

**端點**：`GET /api/v1/reports/invoice-register?from=&to=&format=json|csv|xlsx`（限 MANAGER）。

**口徑（皆有整合測試守護）**：
- 期間一律**台灣日曆日**；`to` 給次日 00:00 或當日 23:59:59 都要涵蓋到最後一天。
- 銷項依**發票日期**歸期；未配號者用建檔時間，否則整批消失。
- 作廢待平台確認（`VOID_PENDING`）期間**發票仍有效**，留在銷項並另列未完成（ADR-019）。
- 開立前就作廢、從未取得字軌號碼的草稿**不是**作廢稅單，不計入作廢合計。
- 折讓需平台核可（G0401 `UPLOADED`）才計入；待送／退回者另列。
- **前期開立、本期作廢**另列一段，不混入本期作廢合計：電子票依 F0501 送出成功時間、
  手開紙本依作廢稽核時間（紙本不排 F0501）；兩者都須曾經配號，草稿不算作廢稅單。
  電子票在「申請作廢」與「平台核可」跨月時，只認平台核可那個月，不重複列。
- **手開紙本**的單在本期有退貨時另列「待調整」（電子端不會有折讓／作廢，docs/36），
  並顯示本期退款金額與筆數；這些仍計入銷項合計，故另給「紙本待調整退款」合計可扣。
- 匯出檔標明店別、期間、產生時間，每列附類別與原交易識別；畫面上的每個合計都要在匯出裡。
