...
> 狀態：**B1–B4 已完成並合併 `main`**；真 R2 備份／下載／解密／throwaway DB 還原及
> 23/23 逐功能演練已通過。正式部署仍須注入 R2 憑證與店外保管的備份口令。
>
...
- **明確不做(單店口徑,Codex 對抗審提出但屬多機/過度工程,單機單店無實益)**:備份/還原的 durable
  全域鎖與 fencing token(單店只有一店＝店級單飛守衛即全域,備份不會併發);還原 VERIFIED 前「起真
  後端跑認證關鍵路徑煙霧」與 PII 解密 canary(同機還原、金鑰在自己 .env,用錯第一次查身分證即發現,
  兩組金鑰店外保管提醒已涵蓋操作面);隔離區/退避/完整 quota/TTL 系統。改以「失敗即刪＋目錄感知修剪
  ＋單飛守衛＋筆數 manifest＋PNG 檔頭驗＋啟動掃除明文＋log 可見」達到單機單店該有的穩健。
...
做法:逐功能比對「備份當下 vs 還原後」的關鍵查詢結果。

> **範圍修正(2026-08-02)**:本節原寫「對還原庫**起一個後端**」與「PII 密文解出明文」,但 §4 的
> 「明確不做」清單(Codex 對抗審後的單店口徑決定)已把**起真後端跑認證關鍵路徑煙霧**與
> **PII 解密 canary** 排除在外。§4 是後來的決定,以 §4 為準;四驗第四項維持連線可用性檢查。
> 兩處衝突至此消除。
...
| 會員/賣方(contacts) | 姓名/電話/角色/點數;PII **密文**逐筆一致(密文與盲索引雜湊)。<br>~~PII 密文解出明文~~ → **已由 §4「明確不做」排除**(同機還原、金鑰在自己 .env,用錯第一次查身分證即發現) |
...
**涵蓋率守衛**:`backend/tests/test_restore_coverage_drift.py` 強制每一張表**要嘛進檢查、
要嘛明寫豁免並附理由**,漏了就測試紅燈——人工清單一定會漂,這是唯一能讓缺口不再重演的作法。

**已知未達成的項目(2026-08-02 盤點,列為待辦而非假裝已做)**:下列 §6.1 要求目前只做到聚合/
雜湊層級,尚未逐一落實。演練綠燈**不代表**這些已驗證:

| 待辦 | 規格要求 | 目前只做到 |
|---|---|---|
| 財務報表 | 對還原庫跑毛利/日結/趨勢,數字一致 | **完全未做** |
| 現金對帳 | 應有現金**公式結果**一致 | 班別數/異動筆數 |
| 交易紀錄 | 抽數筆**逐欄**比對 | 聚合＋狀態雜湊 |
| 寄售結算 | 各結算狀態/金額/**reclaim** | 結算筆數 |
| 電子發票 | 開立/作廢/折讓狀態、**字軌** | 狀態＋作廢原因雜湊、佇列/回執筆數 |
