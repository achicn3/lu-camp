...
- 散裝 remaining_qty / 一般商品 quantity_on_hand 以條件式 UPDATE 原子扣減，不得 < 0。
...
class MarginBreakdown:
    ...
    # （＝recognized_revenue − food_revenue，含二手買斷/寄售抽成/一般商品）。
...
def _sum_campaigns(
    per_line: Iterable[tuple[QuoteLineCampaign, ...]],
) -> list[QuoteLineCampaign]:
    """各行的活動折讓依活動加總（依活動 id 排序）。"""
    totals: dict[int, QuoteLineCampaign] = {}
    for campaigns in per_line:
        for c in campaigns:
            prev = totals.get(c.campaign_id)
            totals[c.campaign_id] = QuoteLineCampaign(
                c.campaign_id,
                c.name,
                c.discount_amount + (prev.discount_amount if prev else Decimal(0)),
            )
...
def _cart_fingerprint(
    ...
    **贈品與折扣納入指紋**：兩者直接改變金額與庫存，同 key 但折扣不同若被當成重放，
    ...
    canonical: dict[str, object] = {
        ...
        "lines": sorted(
            (_line_fingerprint(line) for line in lines),
        ...
        "tenders": (
            ...
            if tenders is None
                ...
                (
                    {
                        "tender_type": t.tender_type.value,
...
class SalesService:
    ...
    async def _validate_display_cart_checkout(
        ...
        *,
        ...
        """確認實際成交與顧客裝置看到的權威購物車完全一致。"""
        persisted_lines = await self._repo.list_lines(sale.id)
        if len(lines) != len(persisted_lines):
            raise SignatureContentMismatch("實際成交明細筆數與客顯購物車不符")
        ...
        for source, persisted in zip(lines, persisted_lines, strict=True):
            ...
            visible = {
                ...
                "original_unit_price": (
                    ...
                    if persisted.original_unit_price is not None
                    else None
                ),
            ...
            actual_items.append(
                {
                    "item_key": self._cart_item_key(source),
                    "line_type": persisted.line_type.value,
                    **visible,
                }
            )
            signed_items.append(visible)
    ...
    async def _bind_store_credit_signature(
        ...
        *,
        ...
        if signature_task_id is None or cart is None:
            raise SignatureContentMismatch("使用購物金必須從 POS 權威購物車送至客顯簽署後再結帳")
        ...
        if task.cart_session_id != cart.id or cart.active_signature_task_id != task.id:
            raise SignatureContentMismatch("簽署任務與目前凍結的購物車不一致")
        content_hash = hashlib.sha256(canonical_json_bytes(task.content)).hexdigest()
        if task.content_sha256 != content_hash:
            raise SignatureContentMismatch("簽署內容雜湊不符，拒絕結帳")
        if task.cart_snapshot_fingerprint != cart.snapshot_fingerprint:
            raise SignatureContentMismatch("簽署綁定的購物車指紋不符，拒絕結帳")
        signed_debit = self._signed_amount(task.content, "store_credit_amount")
        ...
        signed_total = self._signed_amount(task.content, "total")
        if signed_total is None or signed_total != sale_total:
            raise SignatureContentMismatch("結帳總額與已簽確認（本次消費合計）不符，請重新推送簽署")
        expected_lines = task.content.get("items")
        persisted_lines = await self._repo.list_lines(sale.id)
        actual_lines: list[dict[str, object]] = []
        if len(lines) != len(persisted_lines):
            raise SignatureContentMismatch("實際成交明細筆數與簽署快照不符")
        ...
        if expected_lines != actual_lines:
            raise SignatureContentMismatch("實際成交商品、數量、單價或折扣與簽署快照不符")
        frozen_tenders = cart.snapshot.get("tenders")
        actual_tenders = [
            {"tender_type": tender.tender_type.value, "amount": format(tender.amount, "f")}
            for tender in plan
        ]
        ...
        signed_before = self._signed_amount(task.content, "store_credit_balance_before")
        signed_after = self._signed_amount(task.content, "store_credit_balance_after")
    ...
    async def create_sale(
        ...
        *,
        ...
        if cart_session_id is not None:
            ...
            if cart is None:
                raise SignatureContentMismatch("POS 購物車不存在或不屬本店")
            if cart_revision is None or cart.revision != cart_revision:
                raise SignatureContentMismatch(
                    f"POS 購物車版本不符（目前 {cart.revision}），請重新載入"
                )
        ...
        if line_pay_tenders:
            ...
            # idempotency key／一次性付款碼。cart 上的 payment_order_id 是後端保存的
            # 權威原單號；後續仍會 check-first 並核對平台金額，絕不再次 pay。
            reconciled_order_id = (
                cart.payment_order_id
                if cart is not None and cart.payment_order_id is not None
                else None
            )
            reconciled_retry = reconciled_order_id is not None
            if (
                any(t.line_pay_one_time_key is None for t in line_pay_tenders)
                and not reconciled_retry
            ):
                raise InvalidSaleTender("LINE Pay 收款必須帶一次性付款碼（掃客人條碼）")
            if linepay_client is None and reconciled_linepay_result is None:
        ...
        if has_store_credit:
            if signature_task_id is None or cart is None:
                raise SignatureContentMismatch("使用購物金必須由已凍結的客顯購物車完成簽署")
            if (
                cart.status
                not in (
                ...
                or cart.buyer_contact_id != buyer_contact_id
            ):
                raise SignatureContentMismatch("購物車未以此簽署任務凍結，請重新送出簽署")
        elif cart is not None and cart.status not in (
            CartSessionStatus.DRAFT,
            CartSessionStatus.PROCESSING,
        ):
            raise SignatureContentMismatch("一般付款只能結帳可編輯或付款處理中的購物車")
        ...
        if cart is not None:
            await self._validate_display_cart_checkout(
                cart,
                sale=sale,
                lines=lines,
                plan=plan,
        ...
        if cart is not None and cart.status is not CartSessionStatus.PROCESSING:
            cart.status = CartSessionStatus.PROCESSING
            cart.revision += 1
            cart.last_activity_at = datetime.now(UTC)
            cart.updated_at = cart.last_activity_at
            cart.last_changes = []
            await self._session.flush()
        ...
        if cart is not None:
            ...
            cart.status = CartSessionStatus.COMPLETED
            cart.sale_id = sale.id
            cart.active_signature_task_id = None
            cart.completed_at = now
            cart.snapshot = {**cart.snapshot, "member": None}
            cart.payment_checkout_payload = None
            cart.snapshot_fingerprint = hashlib.sha256(
                canonical_json_bytes(cart.snapshot)
            ).hexdigest()
            ...
            await self._display_repo.add(
                CartSessionEvent(
                    ...
                    payload={"sale_id": sale.id, "sale_number": f"#{sale.id}"},
                    actor_user_id=clerk_user_id,
                )
            )
        return sale
    ...
    async def _charge_line_pay(
        ...
        *,
        ...
        order_id = order_id_override or linepay_order_id(
        ...
        if reconciled_result is not None:
            checked = reconciled_result
            if (
                not checked.is_complete
                or checked.transaction_id is None
                or checked.amount != tender.amount
            ):
                raise LinePayChargeFailed("LINE Pay 對帳確認內容不完整或金額不符，拒絕補成立交易")
        else:
            assert client is not None
            if attempt_state is not None:
                attempt_state.status = "POSSIBLE"
        ...
        if checked.is_complete and checked.transaction_id is not None:
            ...
            if tender.line_pay_one_time_key is None:
                if attempt_state is not None:
                    attempt_state.status = "FAILED"
                raise LinePayChargeFailed(
                    "LINE Pay 對帳未確認成功且原付款碼已失效；不可重新扣款，請回付款對帳"
                )
            assert client is not None
            if attempt_state is not None:
                attempt_state.status = "POSSIBLE"
    ...
    async def void_sale(
        ...
        *,
        ...
        # 手開紙本發票不可走平台作廢（docs/36）：**必須是第一道**，早於任何付款判斷。
    ...
    async def quote_sale(
        ...
        *,
        ...
        applied = _sum_campaigns(ql.campaigns for ql in quoted)
        return SaleQuote(
            ...
            total=total,
            campaign_id=(
                max(applied, key=lambda c: c.discount_amount).campaign_id if applied else None
            ),
    ...
    async def _add_priced_line(self, sale_line: SaleLine, disc: _AppliedDiscount) -> SaleLine:
        ...
        saved = await self._repo.add_line(sale_line)
        if disc.allocations:
            await self._repo.add_line_campaigns(
                [
                    SaleLineCampaign(
    ...
    async def _process_basket(
        ...
        sale_line = await self._add_priced_line(
            SaleLine(
                store_id=store_id,
                sale_id=sale_id,
                line_type=SaleLineType.BULK_LOT,
