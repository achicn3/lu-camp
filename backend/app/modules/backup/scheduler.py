...
async def sweep_container_plaintext_on_startup() -> None:
    """啟動時掃除容器內殘留明文 dump（崩潰/中斷留下的整庫明文 PII 不長期滯留,Codex #4）。"""
    try:
        await asyncio.to_thread(_sweep_container_plaintext_sync)
    except Exception:
        logger.warning("startup container plaintext sweep failed", exc_info=True)
